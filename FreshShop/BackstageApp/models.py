...
class Seller(models.Model):
    ...
    picture = models.ImageField(upload_to='BackstageApp/images',verbose_name='用户头像',null=True,blank=True)
...
class Store(models.Model):
    ...
    store_logo = models.ImageField(upload_to='BackstageApp/images',verbose_name='店铺logo')
    ...
    type = models.ManyToManyField(to=StoreType,verbose_name='店铺类型')#与店铺类型建立多对多关系
...
class Goods(models.Model):
    ...
    goods_image = models.ImageField(upload_to='BackstageApp/images',verbose_name='商品图片')
    goods_number = models.IntegerField(verbose_name='商品数量库存')
    goods_description = models.TextField(verbose_name='产品描述')
    goods_date = models.DateField(verbose_name='出厂日期')
    goods_safeDate = models.DateField(verbose_name='保质期')
    store_id = models.ManyToManyField(to=Store,verbose_name="商品店铺")#商品和店铺是多对多关系

#商品图片
class GoodsImg(models.Model):
    img_address = models.ImageField(upload_to='BackstageApp/images',verbose_name='图片地址')
    img_description = models.TextField(max_length=32,verbose_name='图片描述')
    goods_id = models.ForeignKey(to=Goods,on_delete=models.CASCADE,verbose_name='商品id') #商品图片和商品是多对一 的关系
